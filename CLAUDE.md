...
- 入金演出（`add`／`allowance`受取）はコインが降る旧演出をやめ、残高カードを裏面（おさいふ）にフリップし、今回増えた紙幣・硬貨が上からスッと入る（`animateDepositIntoWallet`：`deposit` が返す `tx.coins` の内訳ぶんのタイルを各種類の末尾から拾って隠し、フリップ後に `flyTile(... , {fade:true})` で落とし込む）。演出後はおさいふ面のまま（タップで残高表示へ戻る）。支払い（`spend`）側は従来どおり表面でコインが飛ぶ（`spawnCoins("out")`）＋残高アニメ。
